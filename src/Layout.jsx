...
export default function Layout({ variant = "default" }) {
  const navigate = useNavigate();
  const location = useLocation();
  const isFavourites = location.pathname === "/favourites";
  const showHomeHeader = variant === "home";
  const showDesktopHeroHeader = variant !== "home";
  const showMobileHeader = variant !== "home";

  return (
    <main className="min-h-screen w-full bg-gray-300 dark:bg-gray-900 text-gray-900 dark:text-white">
      {showHomeHeader && <Header />}
      {showDesktopHeroHeader && (
